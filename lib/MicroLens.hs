...
_1 :: Lens (a, x) (b, x) a b
_1 f s = fmap (,snd s) (f (fst s))

-- | A lens focusing on the second element in a pair
_2 :: Lens (x, a) (x, b) a b
_2 f s = fmap (fst s,) (f (snd s))
...
to :: (a -> b) -> Fold a b
to f s t = contramap f (s (f t)) -- TODO: check?
